pipeline {
    ...
    environment {
        ...
        EKS_CLUSTER_NAME = 'jo_cluster' // Replace with your EKS cluster name
    ...
    stages {
        stage('SCM checkout') {
            steps {
                script {
                    git branch: 'main', credentialsId: 'git-cred', url: 'https://github.com/Judah-Joshua/K8s_Real_Project.git'
                }
            }
        }
            ...
            steps {
                script {
                    withCredentials([[
                        ...
                        credentialsId: 'aws-crendentails-jo' // Your Jenkins AWS credentials ID
                    ]]) {
        ...
        stage('Deploy on EKS') {
            steps {
                script {
                    withCredentials([[
                        $class: 'AmazonWebServicesCredentialsBinding',
                        credentialsId: 'aws-crendentails-jo' // Replace with your Jenkins AWS credentials ID
                    ]]) {
